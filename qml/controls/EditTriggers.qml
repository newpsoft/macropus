...
Page {
	...
	property var interfaceModel: TriggerSerial.registry.interfaceList
...
//			WidthConstraint { target: dropList }
			...
			dragKeys: ["application/trigger"]
			cloneElement: new Model.Trigger()
			contentComponent: InterfaceView {
				...
				interfaceRole: "itrigger"
				Binding on itemModel {
					value: control.model.get(index)
				}

				Binding on interfaceIndex {
					value: item.itemModel && TriggerSerial.registry.indexOf(item.itemModel.itrigger)
				}
			}
		}
	}
